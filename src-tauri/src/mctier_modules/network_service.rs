use crate::mctier_modules::error::AppError;
use crate::mctier_modules::resource_manager::ResourceManager;
...
impl NetworkService {
    ...
    fn apply_advanced_config(
        ...
        config: &crate::mctier_modules::config_manager::EasyTierAdvancedConfig,
    ) {
    ...
    pub async fn start_easytier_with_config(
        ...
        player_name: String,
        app_handle: &tauri::AppHandle,
        global_config_param: Option<Option<crate::mctier_modules::config_manager::EasyTierAdvancedConfig>>,
        lobby_config_param: Option<Option<crate::mctier_modules::config_manager::EasyTierAdvancedConfig>>,
    ) -> Result<String, AppError> {
        ...
        #[cfg(windows)]
        {
            ...
            // -------- 添加 Windows 防火墙规则（子网级，覆盖所有网络配置文件） --------
            // 使用子网级规则替代端口级规则，覆盖所有 P2P 联机游戏（MC、泰拉瑞亚、CS:GO 等）
            // 游戏 LAN 模式使用随机动态端口，端口级规则无法覆盖
            // remoteip=10.126.126.0/24 限制只有虚拟子网流量放行，不影响安全性
            log::info!("🛡️ 正在添加 Windows 防火墙规则（子网级）以允许 P2P 通信...");
            
            // 规则1：protocol=any，放行虚拟子网全部入站（UDP + TCP + ICMP，所有端口）
            let subnet_rule_name = "MCTier P2P Network";
            let subnet_result = std::process::Command::new("netsh")
                .args(&[
                    "advfirewall", "firewall", "add", "rule",
                    &format!("name={}", subnet_rule_name),
                    "dir=in",
                    "action=allow",
                    "protocol=any",
            ...
            match subnet_result {
                Ok(output) => {
                    if output.status.success() {
                        log::info!("✅ 子网通杀规则已添加: {}", subnet_rule_name);
                    } else {
                        let stderr = String::from_utf8_lossy(&output.stderr);
                        log::info!("📝 子网通杀规则可能已存在: {}", stderr);
                    }
                }
                Err(e) => {
                    log::warn!("⚠️ 无法添加子网通杀规则(非致命错误): {}", e);
                }
            }
            
            // 规则2：protocol=tcp（冗余），部分 Windows 配置下 protocol=any 对 TCP 不完全生效
            let tcp_rule_name = "MCTier P2P TCP";
            let tcp_result = std::process::Command::new("netsh")
                .args(&[
                    "advfirewall", "firewall", "add", "rule",
                    &format!("name={}", tcp_rule_name),
                    "dir=in",
                    "action=allow",
                    "protocol=tcp",
                    "remoteip=10.126.126.0/24",
            ...
            match tcp_result {
                Ok(output) => {
                    if output.status.success() {
                        log::info!("✅ TCP通杀规则已添加: {}", tcp_rule_name);
                    } else {
                        let stderr = String::from_utf8_lossy(&output.stderr);
                        log::info!("📝 TCP通杀规则可能已存在: {}", stderr);
                    }
                }
                Err(e) => {
                    log::warn!("⚠️ 无法添加TCP通杀规则(非致命错误): {}", e);
                }
            }
            
            // ======== 出站规则（dir=out）：修复 UDP 组播单向不通的问题 ========
            // MC LAN 发现使用 UDP 组播 (224.0.2.60:4445)，目标地址不在 10.126.126.0/24 内
            // 出站规则必须用 localip（限制本机源IP在虚拟子网），而非 remoteip（目标IP）
            // "从虚拟IP发出的任何流量都放行" — 包括发送到 224.0.2.60 的 MC 组播
            
            // 规则3：protocol=any，放行从虚拟子网发出的全部出站流量
            let out_subnet_rule_name = "MCTier P2P Network Out";
            let out_subnet_result = std::process::Command::new("netsh")
                .args(&[
                    "advfirewall", "firewall", "add", "rule",
                    &format!("name={}", out_subnet_rule_name),
                    "dir=out",
                    "action=allow",
                    "protocol=any",
                    "localip=10.126.126.0/24",
                    "profile=any",
                    "enable=yes",
                ])
                .creation_flags(CREATE_NO_WINDOW)
            ...
            match out_subnet_result {
                Ok(output) => {
                    if output.status.success() {
                        log::info!("✅ 出站通杀规则已添加: {}", out_subnet_rule_name);
                    } else {
                        let stderr = String::from_utf8_lossy(&output.stderr);
                        log::info!("📝 出站通杀规则可能已存在: {}", stderr);
                    }
                }
                Err(e) => {
                    log::warn!("⚠️ 无法添加出站通杀规则(非致命错误): {}", e);
                }
            }
            
            // 规则4：protocol=tcp（冗余）出站
            let out_tcp_rule_name = "MCTier P2P TCP Out";
            let out_tcp_result = std::process::Command::new("netsh")
                .args(&[
                    "advfirewall", "firewall", "add", "rule",
                    &format!("name={}", out_tcp_rule_name),
                    "dir=out",
                    "action=allow",
                    "protocol=tcp",
                    "localip=10.126.126.0/24",
            ...
            match out_tcp_result {
                Ok(output) => {
                    if output.status.success() {
                        log::info!("✅ 出站TCP规则已添加: {}", out_tcp_rule_name);
                    } else {
                        let stderr = String::from_utf8_lossy(&output.stderr);
                        log::info!("📝 出站TCP规则可能已存在: {}", stderr);
                    }
                }
                Err(e) => {
                    log::warn!("⚠️ 无法添加出站TCP规则(非致命错误): {}", e);
                }
            }
        }
        ...
        *self.rpc_port.lock().await = Some(rpc_port);
        ...
        // 读取高级功能配置
        use tauri::Manager;
        use crate::mctier_modules::config_manager::EasyTierAdvancedConfig;
        ...
        let (global_config, lobby_config) = if global_config_param.is_some() || lobby_config_param.is_some() {
            ...
            let state = app_handle.state::<crate::mctier_modules::tauri_commands::AppState>();
    ...
    /// 从输出行中提取 IP 地址
    /// 
    /// 只匹配 EasyTier 虚拟网段的 IP（10.126.x.x），排除物理网卡的内网 IP（192.168.x.x、172.16-31.x.x）
    /// 否则当 EasyTier 日志中先出现物理网卡 IP 时，会错误地将其作为"虚拟 IP"
        ...
        for cap in ip_pattern.captures_iter(line) {
            if let Some(ip_match) = cap.get(1) {
                ...
                if Self::is_valid_ip(ip) {
                    // 只接受 EasyTier 虚拟网段 IP (10.126.0.0/16)，排除本地回环
                    // 不能接受所有私有IP，因为 EasyTier 日志中常包含物理网卡的 192.168.x.x
                    let parts: Vec<u8> = ip.split('.')
                        .filter_map(|p| p.parse::<u8>().ok())
                        .collect();
                    if parts.len() == 4 && parts[0] == 10 && parts[1] == 126 && !Self::is_loopback(ip) {
                        log::info!("从 EasyTier 输出中提取到虚拟IP: {}", ip);
                        log::info!("输出行内容: {}", line);
