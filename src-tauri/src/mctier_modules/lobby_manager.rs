...
use crate::mctier_modules::hosts_manager::HostsManager;
...
/// EasyTier 高级配置参数
#[derive(Debug, Clone)]
pub struct EasyTierConfigParams {
    /// 全局 EasyTier 高级配置
    pub global_config: Option<crate::mctier_modules::config_manager::EasyTierAdvancedConfig>,
    /// 大厅 EasyTier 高级配置
    pub lobby_config: Option<crate::mctier_modules::config_manager::EasyTierAdvancedConfig>,
}
...
impl LobbyManager {
    ...
    ///
    /// # 参数
    /// * `params` - 大厅参数（名称、密码、玩家名称等）
    /// * `network_service` - 网络服务引用（用于连接 EasyTier）
    /// * `app_handle` - Tauri 应用句柄
    /// * `config` - EasyTier 高级配置参数
    ///
    /// # 返回
    /// * `Ok(Lobby)` - 成功创建的大厅信息
    /// * `Err(LobbyError)` - 创建失败
    pub async fn create_lobby_with_config(
        &mut self,
        params: LobbyParams,
        network_service: &crate::mctier_modules::network_service::NetworkService,
        app_handle: &tauri::AppHandle,
        config: EasyTierConfigParams,
        ...
        // 验证输入
        Self::validate_lobby_name(&params.name)?;
        Self::validate_password(&params.password)?;
        Self::validate_input(&params.player_name, "玩家名称")?;
        Self::validate_input(&params.server_node, "服务器节点")?;

        log::info!("正在创建大厅: {}, 使用域名: {}, 虚拟域名: {:?}", params.name, params.use_domain, params.virtual_domain);

        // 构建 EasyTier 网络凭证
        // 使用 "MCTier-" + 大厅名称作为网络号，实现大厅隔离
        let network_name = format!("MCTier-{}", params.name);
        let network_key = params.password.clone();

        log::info!("EasyTier 网络号: {}", network_name);

        let normalized_server_node = Self::normalize_server_node(&params.server_node);
        ...
        let virtual_ip = network_service
            .start_easytier_with_config(
                ...
                Some(config.lobby_config),
            )
            .await
            .map_err(|e| LobbyError::NetworkError(e.to_string()))?;

        // 使用传入的虚拟域名，如果没有则生成默认的（格式：玩家名.mct.net）
        let final_virtual_domain = if let Some(domain) = params.virtual_domain {
            if domain.is_empty() {
                Some(format!("{}.mct.net", params.player_name))
            } else {
                Some(domain)
            }
        } else {
        ...
        if params.use_domain {
            log::info!("启用域名访问，创建HostsManager...");
            let hosts_manager = HostsManager::new(&params.name);
        ...
        let lobby = Lobby::new(
            ...
            Some(params.use_domain),
            Some(params.signaling_server),
        );

        // 创建当前玩家
        let player = Player::new(params.player_name, virtual_ip.clone());
    ...
    pub async fn create_lobby(
        ...
        let virtual_ip = network_service
            .start_easytier(network_name, network_key, normalized_server_node, params.player_name.clone(), app_handle)
            .await
            .map_err(|e| LobbyError::NetworkError(e.to_string()))?;
    ...
    pub async fn join_lobby_with_config(
        ...
        log::info!("正在加入大厅: {}, 使用域名: {}, 虚拟域名: {:?}", params.name, params.use_domain, params.virtual_domain);
    ...
    pub async fn join_lobby(
        ...
        // 创建当前玩家
        let player = Player::new(params.player_name, virtual_ip);
