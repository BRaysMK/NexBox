...
use tauri::Emitter;
use tauri::Manager;
use crate::mctier_modules::app_core::{AppCore, AppState as CoreAppState};
use crate::mctier_modules::lobby_manager::{Lobby, Player};
use crate::mctier_modules::voice_service::AudioDevice;
use crate::mctier_modules::config_manager::UserConfig;
use std::sync::Arc;
...
pub async fn create_lobby(
    ...
    match lobby_mgr.create_lobby_with_config(
        crate::mctier_modules::lobby_manager::LobbyParams {
            name,
            password,
            player_name: player_name.clone(),
            server_node,
            signaling_server: signaling_server.clone(),
            use_domain: use_domain.unwrap_or(false),
            virtual_domain,
        },
        &network_svc,
        &app_handle,
        crate::mctier_modules::lobby_manager::EasyTierConfigParams {
            global_config,
            lobby_config,
        },
    ).await {
...
/// 强制退出应用（由前端在完成联机服务清理后调用）
/// 
/// 执行与 ExitRequested 相同的清理逻辑，然后强制退出进程
#[tauri::command]
pub async fn force_exit_app(app: tauri::AppHandle) -> Result<(), String> {
    log::info!("强制退出应用");
    
    crate::sensor::stop_sensor_process(&app);
    crate::hardware::cleanup_hardware_cache();
    crate::display_filter::cleanup();
    crate::overlay_panel::cleanup();
    crate::crosshair::cleanup();
    crate::tray::cleanup();
    crate::hotkey::cleanup(&app);
...
pub async fn save_window_position(
    ...
    use crate::mctier_modules::config_manager::WindowPosition;
...
pub async fn set_window_opacity(
    ...
    // 修复：移除 Windows SetLayeredWindowAttributes / WS_EX_LAYERED 的调用
    // 部分电脑在动态添加 WS_EX_LAYERED 样式后会出现渲染异常导致窗口变透明
    // 前端已经通过 CSS rgba() 处理了视觉透明度效果
    
    // 发送事件到前端，让前端通过 CSS 控制UI透明度
    window
...
pub async fn send_signaling_message(
    ...
    let p2p_message = match msg_type {
        "offer" => {
            ...
            crate::mctier_modules::p2p_signaling::P2PMessage::Offer { from, sdp }
        }
        "answer" => {
            let sdp = message.get("sdp").and_then(|v| v.as_str()).unwrap_or("").to_string();
            crate::mctier_modules::p2p_signaling::P2PMessage::Answer { from, sdp }
        }
        "ice-candidate" => {
            let candidate = message.get("candidate").and_then(|v| v.as_str()).unwrap_or("").to_string();
            crate::mctier_modules::p2p_signaling::P2PMessage::IceCandidate { from, candidate }
        }
        _ => {
...
pub async fn broadcast_status_update(
    ...
    // 创建状态更新消息
    let message = crate::mctier_modules::p2p_signaling::P2PMessage::StatusUpdate {
        player_id,
        mic_enabled,
    };
...
pub async fn force_stop_easytier(state: State<'_, AppState>) -> Result<(), String> {
    ...
    match network_svc.stop_easytier().await {
        ...
        Err(e) => {
            log::warn!("?? 强制停止EasyTier进程时出现警告: {}", e);
...
pub async fn add_player_domain(
    ...
    let hosts_manager = if let Some(hm) = manager.get_hosts_manager() {
        ...
        let new_hosts_manager = crate::mctier_modules::hosts_manager::HostsManager::new(&lobby_name);
...
pub async fn remove_player_domain(
    ...
    if let Some(hosts_manager) = manager.get_hosts_manager() {
        ...
        log::info!("?? HostsManager不存在，跳过删除域名映射");
        Ok(())
    }
}
...
use crate::mctier_modules::file_transfer::{SharedFolder, FileInfo as FileTransferFileInfo};
...
pub async fn get_remote_shares(peer_ip: String) -> Result<Vec<SharedFolder>, String> {
    ...
    let client = reqwest::Client::builder()
        .no_proxy() // 禁用代理，防止系统代理拦截虚拟IP P2P请求
...
use crate::mctier_modules::chat_service::{ChatMessage as ChatServiceMessage, MessageType, SendMessageRequest};
            ...
            match client_clone.post(&url_clone).json(&request).send().await {
                Ok(response) => {
                    ...
                    if response.status().is_success() {
                        ...
                        log::warn!("?? 发送消息失败 ({}): HTTP {}", url_clone, response.status());
                    }
                }
                Err(e) => {
                    let elapsed = start.elapsed();
                    log::warn!("?? 发送消息失败 ({}, 耗时: {:?}): {}", url_clone, elapsed, e);
                }
            }
        });
            ...
            Ok(response) => {
                if response.status().is_success() {
                    match response.json::<Vec<ChatServiceMessage>>().await {
                        ...
                        Err(e) => {
                            log::warn!("?? 解析消息失败 ({}): {}", peer_ip, e);
                        }
                    }
                } else {
                    log::warn!("?? HTTP请求失败 ({}): 状态码 {}", peer_ip, response.status());
                }
            }
            Err(e) => {
                // 超时或连接失败不打印警告，避免日志刷屏
                log::debug!("?? 获取消息失败 ({}): {}", peer_ip, e);
            }
        }
    }
...
pub async fn save_settings(
    ...
    use crate::mctier_modules::config_manager::{AutoLobbyConfig, EasyTierNode};
    log::info!("保存设置: auto_startup={}, auto_lobby_enabled={}, use_private_server={}, always_on_top={:?}, remember_window_position={:?}, voice_volume={:?}, mic_hotkey={:?}, global_mute_hotkey={:?}, push_to_talk_hotkey={:?}, enable_exit_node={:?}, subnet_proxy_cidrs={:?}, virtual_domain={:?}", 
        auto_startup, auto_lobby_enabled, use_private_server, always_on_top, remember_window_position, voice_volume, mic_hotkey, global_mute_hotkey, push_to_talk_hotkey, enable_exit_node, subnet_proxy_cidrs, virtual_domain);
        ...
        cfg_mgr.update_config(|config| {
            ...
            if let Some(enable) = enable_exit_node {
                if config.exit_node_config.is_none() {
                    config.exit_node_config = Some(crate::mctier_modules::config_manager::ExitNodeConfig::default());
                }
...
pub async fn save_exit_node_advanced_config(
    ...
    use crate::mctier_modules::config_manager::PortForwardRule;
