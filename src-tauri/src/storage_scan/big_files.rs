...
pub fn scan(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // ========================================================================
        // 尝试 MFT 全量直读引擎(管理员 + NTFS 时秒级完成,参考 light-c-main)
        // ========================================================================
        {
            use crate::storage_scan::big_files_engine::mft_core;
            ...
            if is_admin && is_ntfs_drive {
                ...
                match crate::storage_scan::big_files_engine::mft_bigfiles::scan_top_files_via_mft(
                    top_n,
                    drive_letter,
                    |progress| {
                        let _ = window.emit(
        ...
        // ========================================================================
        // 降级:jwalk 并行遍历(普通权限 / 非 NTFS)
        // ========================================================================
        let heap: std::sync::Mutex<BinaryHeap<Reverse<LargeFileEntry>>> =
            std::sync::Mutex::new(BinaryHeap::new());
        let file_count = AtomicU64::new(0);
        let last_emit = Instant::now();
        let scan_started = Instant::now();

        // jwalk 并行遍历磁盘,显著加快大文件扫描速度(相比单线程 walkdir)。
        // process_read_dir 负责剪枝:跳过回收站、系统卷信息、Windows winsxs 等无用目录。
                            ...
                            if entry.file_type.is_dir() {
                                ...
                                if name == "winsxs" {
                                    let parent = entry
                                        ...
                                        .to_string_lossy()
                                        .to_lowercase();
                                    if parent.ends_with("\\windows") || parent.ends_with("/windows") {
                                        return None;
                                    }
                                }
                            }
            ...
            .filter(|e| e.file_type().is_file())
        {
            if is_cancelled() {
                log::info!("大文件扫描被用户取消");
                let _ = window.emit("large-file-scan:cancelled", ());
                let heap = heap.into_inner().unwrap();
                let mut results: Vec<LargeFileEntry> =
                    heap.iter().map(|item| item.0.clone()).collect();
            ...
            if let Ok(metadata) = entry.metadata() {
                ...
                let count = file_count.fetch_add(1, std::sync::atomic::Ordering::Relaxed) + 1;

                if last_emit.elapsed().as_millis() >= 200 || count % 1000 == 0 {
                    let progress = LargeFileScanProgress {
                        current_path: path_str.clone(),
                        scanned_count: count,
                        found_count: heap.lock().unwrap().len(),
                        backend: "jwalk".into(),
                        stage: "jwalk".into(),
        ...
        let heap = heap.into_inner().unwrap();
        let mut results: Vec<LargeFileEntry> = heap.iter().map(|item| item.0.clone()).collect();
        results.sort_by(|a, b| b.size.cmp(&a.size));
    ...
    {
        let _ = (window, top_n, drive_letter);
...
pub(crate) fn compute_source_label(path: &str) -> String {
    ...
    if ["zip", "rar", "7z", "tar", "gz", "bz2", "xz", "zst", "cab"].contains(&ext) {
