...
const FILE_PATH = path.join(__dirname, '射手步枪.txt');
...
async function main() {
  ...
  for (const rawLine of lines) {
    ...
    if (costMatch) {
      ...
      rows.push([5, currentWeapon, code, cost, '木流', 'approved', 0]);
    } else {
      rows.push([5, currentWeapon, line, 0, '木流', 'approved', 0]);
    }
  }
  ...
  console.log(`已插入 ${rows.length} 条射手步枪记录，上传者：木流`);
  for (const r of rows) {
    console.log(`  ${r[1]} | ${r[3]}💰`);
  }
}
