...
interface AppStore {
  ...
  // ==================== 退出控制 ====================
  /** 是否收到后端退出请求（窗口关闭/托盘退出），触发 MiniWindow 的 handleLeaveLobby */
  pendingLeaveLobby: boolean;
  /** 设置 pendingLeaveLobby */
  setPendingLeaveLobby: (value: boolean) => void;
  /** 退出大厅后是否关闭软件（托盘退出=true，X关闭=false） */
  shouldExitAfterLeave: boolean;
  /** 设置 shouldExitAfterLeave */
  setShouldExitAfterLeave: (value: boolean) => void;
}
...
export const useAppStore = create<AppStore>()(
  devtools(
    (set, get) => ({
      ...
      // ==================== 退出控制 ====================
      setPendingLeaveLobby: (value: boolean) => {
        set({ pendingLeaveLobby: value }, false, 'setPendingLeaveLobby');
      },

      setShouldExitAfterLeave: (value: boolean) => {
        set({ shouldExitAfterLeave: value }, false, 'setShouldExitAfterLeave');
      },
    }),
    {
