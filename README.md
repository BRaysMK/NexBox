...
见本仓库 [Releases](https://github.com/BRaysMK/NexBox/releases)（`v8.0.9+08182045` 附完整 exe）。
...
NexBox 自带 **WinDivert64.sys**（网络数据包捕获驱动，随 PawnIO 外设驱动安装包分发），
部分杀毒软件（如卡巴斯基）会对**内核驱动**报"可疑软件"。

- ✅ 该文件**有合法数字签名**（签发者：成都密思听科技有限公司，时间戳：Sectigo），SHA256：`8DA085332782708D8767BCACE5327A6EC7283C17CFB85E40B03CD2323A90DDC2`
- 📖 WinDivert 是开源库（LGPL-3.0），被大量网络工具/外设软件使用，杀软因「内核驱动 + 外设工具」组合容易启发式误报
- 🔧 如果卡巴斯基拦截：加入信任区（设置 → 威胁和排除项 → 排除文件/文件夹），或将 NexBox 安装目录加入排除即可
- ⚠️ 若你在**非官方渠道**（非本仓库 Releases）下载到无签名或哈希不一致的版本，请勿信任，立即删除
